  ...
  return info ? (
    ...
    >
      ...
      <nav className="h-[10vh] w-full text-zinc-100 flex items-center gap-3 sm:gap-5 text-xl">
        <button onClick={() => navigate(-1)} className="w-8 h-8 rounded-full hover:bg-[#6556CD] flex items-center justify-center transition-all duration-300 cursor-pointer" title="Go Back">
          <i className="ri-arrow-left-line text-lg" />
        </button>
        <Link to="/home" className="w-8 h-8 rounded-full hover:bg-[#6556CD] flex items-center justify-center transition-all duration-300" title="Home">
      ...
      <div className="w-full flex flex-col md:flex-row gap-8 md:gap-0">
        ...
        <div className="content md:ml-[5%] text-white">
          ...
          <h1 className="text-xl sm:text-2xl mb-3 mt-5">Movie Translated</h1>
      ...
      <HorizontalCards data={info.recommendations.length > 0 ? info.recommendations : info.similar} media_type="movie" />
      <Outlet />
    </div>
  ) : (
    <Loading />
