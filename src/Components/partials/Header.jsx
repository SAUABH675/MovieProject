...
const Header = ({data}) => {
    ...
    className="w-full h-[50vh] flex flex-col justify-end items-start p-[5%]">
        ...
        <p className="w-[70%] mt-3 mb-3 text-white">
            ...
            <Link to={`/${data.media_type || "movie"}/details/${data.id}`} className="text-blue-400">more</Link> 
        </p>
        <p className="text-white">
            <i className="text-yellow-500 ri-alarm-fill"></i> 
            {data.release_date || "No Information"}
            <i className="ml-5 text-yellow-500 ri-album-fill"></i> 
            {data.media_type?.toUpperCase()}
        </p>
        <Link to={`/${data.media_type}/details/${data.id}/trailer`} className="hover:bg-[#7c6de0] mt-5 bg-[#6556CD] p-4 rounded text-white font-semibold">Watch Trailer</Link>
    </div>
};
